...
if curl --output /dev/null --silent --head --fail "$FILE_URL"; then
    ...
    sudo echo "deb [arch=arm64 signed-by=/usr/share/keyrings/bazel-archive-keyring.gpg] https://storage.googleapis.com/bazel-apt stable jdk1.8" | tee /etc/apt/sources.list.d/bazel.list
