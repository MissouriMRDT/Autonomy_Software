...
FILE_URL="https://github.com/MissouriMRDT/Autonomy_Packages/raw/main/qt6/arm64/qt6_${QT6_VERSION}_arm64.deb"
...
if [[ "$DOWNLOAD_LATEST" == true ]]; then
    ...
    curl -L $FILE_URL --output /tmp/pkg/deb/qt6_${QT6_VERSION}_arm64.deb
...
if [[ "$FORCE_BUILD" == false ]] && curl --output /dev/null --silent --head --fail "$FILE_URL"; then
    ...
    if [[ "$CHECK_PACKAGE" == true ]]; then
        ...
        mkdir -p /tmp/pkg/qt6_${QT6_VERSION}_arm64/usr/local
        mkdir -p /tmp/pkg/qt6_${QT6_VERSION}_arm64/DEBIAN
        ...
        } > /tmp/pkg/qt6_${QT6_VERSION}_arm64/DEBIAN/control
        ...
        cmake -G Ninja \
            ...
            -DCMAKE_INSTALL_PREFIX=/tmp/pkg/qt6_${QT6_VERSION}_arm64/usr/local \
            ..
        ...
        cmake -G Ninja \
            -DCMAKE_BUILD_TYPE=Release \
            -DCMAKE_INSTALL_PREFIX=/tmp/pkg/qt6_${QT6_VERSION}_arm64/usr/local \
            -DCMAKE_PREFIX_PATH=/tmp/pkg/qt6_${QT6_VERSION}_arm64/usr/local \
            ..
        ...
        # Build deb package
        dpkg --build /tmp/pkg/qt6_${QT6_VERSION}_arm64

        # Create Package Directory if it doesn't exist
        mkdir -p /tmp/pkg/deb

        # Copy Package
        cp /tmp/pkg/qt6_${QT6_VERSION}_arm64.deb /tmp/pkg/deb/qt6_${QT6_VERSION}_arm64.deb
    fi
fi
