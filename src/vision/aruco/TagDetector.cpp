...

{
    ...
    this->SetMainThreadName("TagDet" + m_szCameraName.substr(m_szCameraName.size() > 9 ? m_szCameraName.size() - 9 : 0));
...
bool TagDetector::LoadLatestCameraFrames()
{
    ...
    if (m_bUsingZedCamera)
    {
        ...
        if (m_bUsingGpuMats)
        {
            ...
            // can publish a new grab in between the two loads above and hand us a frame from
            ...
            // path. A download has to materialise host memory, so unlike the CPU path there is
            // no way to avoid a copy here.
        ...
        else
        {
            ...
            // reference, so this shares the pixels rather than copying them, and the snapshots
            // above keep those pixels alive for the whole pass.
            //
