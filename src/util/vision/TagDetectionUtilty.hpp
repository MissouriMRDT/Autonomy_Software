...
namespace tagdetectutils
{
    ...
    inline void LoadDetectedArucoTags(std::vector<arucotag::ArucoTag>& vDetectedTags, const std::vector<TagDetector*>& vTagDetectors, bool bUnique = false)
    {
        ...
        std::vector<std::vector<arucotag::ArucoTag>*> vDetectedTagBuffers(siNumTagDetectors, new std::vector<arucotag::ArucoTag>());

        // Initialize vectors to stored detected tags
        std::vector<std::future<bool>> vDetectedTagsFuture;
        vDetectedTagsFuture.reserve(siNumTagDetectors);
        ...
        for (std::vector<arucotag::ArucoTag>* pBuffer : vDetectedTagBuffers)
        {
            delete pBuffer;
        }
     ...
     * @date 2024-03-07
     ******************************************************************************/
    inline void LoadDetectedTensorflowTags(std::vector<tensorflowtag::TensorflowTag>& vDetectedTags, const std::vector<TagDetector*>& vTagDetectors, bool bUnique = false)
    {
        // Number of tag detectors.
        size_t siNumTagDetectors = vTagDetectors.size();
        ...
        // Initialize vectors to store detected tags temporarily.
        // Using pointers the interference between vectors being updated across different threads should be minimal.
        std::vector<std::vector<tensorflowtag::TensorflowTag>*> vDetectedTagBuffers(siNumTagDetectors, new std::vector<tensorflowtag::TensorflowTag>());
        ...
        // Deallocate the buffer.
        for (std::vector<tensorflowtag::TensorflowTag>* pBuffer : vDetectedTagBuffers)
        {
            delete pBuffer;
        }
