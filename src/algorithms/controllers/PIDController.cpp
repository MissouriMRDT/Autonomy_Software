...
namespace controllers
{
    ...
    /**************************************************************************
     * @brief Calculate the next control output given the current veloctiy and a goal
     *      velocity. 
     *
     * @param dCurrVelo - The current actual velocity of the system.
     * @param dGoalVelo - The desired goal velocity of the controller.
     * @return double - The resultant PID controller output.
     *
     * @author ajbuter (aiden.buter@gmail.com)
     * @date 2026-09-14
     **************************************************************************/
    double PIDController::CalculateVelocity(const double dCurrVelo, const double dGoalVelo)
    {
        // Create instance variables.
        double dPTermOutput;
        double dITermOutput;
        ...
        // Update setpoint member variable.
        m_dSetpoint = dGoalVelo;

        // Apply the sliding setpoint range adjust if necessary.
        if (m_dMaxSetpointDifference != 0)
        {
            // Clamp current setpoint value within a certain range of the current velocity.
            m_dSetpoint = numops::Clamp(m_dSetpoint, dCurrVelo - m_dMaxSetpointDifference, dCurrVelo + m_dMaxSetpointDifference);
        }

        // Determine error from setpoint.
        double dError = m_dSetpoint - dCurrVelo;
        // Check if the controller is within the tolerance.
        if (numops::Bounded(dError, -m_dTolerance, m_dTolerance))
        {
            // Reset the error sum.
        ...
        // Calculate feedforward term.
        dFFTermOutput = m_dKff * dGoalVelo;
        // Calculate proportional term.
        dPTermOutput = m_dKp * dError;
        // Check if this is the first time doing a calculation.
        if (m_bFirstCalculation)
        ...
        /**************************************************************************
     * @brief Calculate the next control output given the current velocity and
     *      using the previously set setpoint.
     *
     * @param dCurrVelo - The current actual velocity of the system.
     * @return double - The resultant PID controller output.
     *
     ...
     * @date 2026-09-14
     **************************************************************************/
    double PIDController::CalculateVelocity(const double dCurrVelo)
    {
        // Calculate and return the output from the PIDController using the same setpoint.
        return this->CalculateVelocity(dCurrVelo, m_dSetpoint);
    }
