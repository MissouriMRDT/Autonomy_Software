...
class AutonomyThread
{
    public:
        ...
        // Define an enum for storing thread priority levels. These are used as an abstraction layer for the BS::pr enum, so we don't have to include the thread pool
        // header in child classes just to set priorities.
        enum class AutonomyThreadPriority
        {
            eLowest = BS::pr::lowest,     // Thread will be scheduled less often and may not be able to reach the max IPS if the system is under heavy load.
            eLow    = BS::pr::low,        // Thread will be scheduled less often and may not be able to reach the max IPS if the system is under heavy load.
            eNormal = BS::pr::normal,     // Default. Thread will be scheduled normally and should be able to reach the max IPS if the system is not under heavy load.
            eHigh   = BS::pr::high,       // Thread will be scheduled more often and should be able to reach the max IPS even if the system is under
                                          // heavy load, but it may starve other threads of CPU time.
            eHighest = BS::pr::highest    // Thread will be scheduled as often as possible and should be able to reach the max IPS even if the system is under heavy load.
        };
        ...
        void Start()
        {
            ...
            std::future<void> fuMainReturn = m_thMainThread.submit_task(
                [this]()
                {
                    // Note: BS::this_thread::set_os_thread_priority was completely removed in BS v5.1.0.
                    // Priority is kept in memory but not applied to OS threads natively by the library anymore.
                    this->RunThread(m_bStopThreads);
                });
        ...
        /******************************************************************************
         * @brief Set the OS priority for the main continuous thread.
         *
         * @param thPriority - The OS thread priority to set.
         *
         ...
         * @param thPriority - The OS thread priority to set.
         *
         * @note This immediately resets the pool and applies the priority.
         *
         * @author clayjay3 (claytonraycowen@gmail.com)
         * @date 2026-03-21
         ******************************************************************************/
    ...
    protected:
        ...
        {
            // Create new thread pool (no specific template flags needed for basic detaching).
