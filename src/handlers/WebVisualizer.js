...
window.modifyColor = function (element) {
    ...
    colorPicker.style.left = (bound.left + 150) + "px";
    colorPicker.style.top = (bound.bottom) + "px";
    ...
    const elementID_Number = parseInt(elementID.match(/\d.*/)?.[0]);
    const elementSpan_Name = elementID_Number < 10 ? stateColors[elementID_Number].name : detectColors[elementID_Number].name;

    const label = document.createElement("label");
    label.innerText = shortenString(elementSpan_Name, 10);
    label.style.position = "fixed";
    label.style.left = (bound.left + 153) + "px";
    label.style.top = (bound.bottom + 6) + "px";
    label.style.color = invertHex(colorPicker.value);
    ...
    colorPicker.addEventListener("input", function () {
        stateColors[elementID_Number] ? stateColors[elementID_Number].color = colorPicker.value : detectColors[elementID_Number].color = colorPicker.value;
        element.style.backgroundColor = stateColors[elementID_Number] ? stateColors[elementID_Number].color : detectColors[elementID_Number].color;
        label.style.color = invertHex(colorPicker.value);
    });
...
function invertHex(hex) {
    hex = hex.replace('#', '');
    const regexNum = /[0-9]/;
    const regexLetter = /[a-fA-F]/;

    let newHex = "";
    for (let i = 0; i < hex.length; i++) {
        let value;
        if (regexNum.test(hex[i])) {
            value = Math.abs(hex[i] - 15);
            if (value >= 10) {
                value = String.fromCharCode(value + 87); // Convert to letter a-f
            }
            newHex += value;
        } else if (regexLetter.test(hex[i])) {
            value = hex[i].toLowerCase().charCodeAt(0) - 87; // Convert letter to number
            newHex += 15 - value;
        }
    }
    return "#" + newHex;
