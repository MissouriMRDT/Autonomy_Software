...
/******************************************************************************
 * @brief Loads the HTML template and embeds the CSS and JS into it.
 *
 * @return std::string - The fully assembled HTML content as a string.
 *
...
{
    // Define paths to the source assets.
    const std::string htmlPath = "/workspaces/Autonomy_Software/src/handlers/WebVisualizer.html";
    const std::string jsPath   = "/workspaces/Autonomy_Software/src/handlers/WebVisualizer.js";
    const std::string cssPath  = "/workspaces/Autonomy_Software/src/handlers/WebVisualizer.css";

    // Load each asset from disk.
    std::vector<char> htmlData = LoadFileToBuffer(htmlPath);
    std::vector<char> jsData   = LoadFileToBuffer(jsPath);
    std::vector<char> cssData  = LoadFileToBuffer(cssPath);

    // Fail if any assets fail to load.
    if (htmlData.empty() || cssData.empty() || jsData.empty())
    {
        LOG_ERROR(logging::g_qSharedLogger, "VisualizationHandler: One or more visualizer assets failed to load. Aborting HTML assembly.");
        return "";
    }

    // Convert the raw byte buffers into strings.
    std::string rawHtml(htmlData.begin(), htmlData.end());
    std::string rawJs(jsData.begin(), jsData.end());
    std::string rawCss(cssData.begin(), cssData.end());

    // Substitute the CSS placeholder.
    const std::string cssPlaceholder = "CSS_PLACEHOLDER";
    size_t cssPos                    = rawHtml.find(cssPlaceholder);
    if (cssPos != std::string::npos)
    {
        rawHtml.replace(cssPos, cssPlaceholder.length(), rawCss);
    }
    else
    {
        LOG_ERROR(logging::g_qSharedLogger, "VisualizationHandler: CSS_PLACEHOLDER not found in WebVisualizer.html.");
        return "";
    }

    // Substitute the JS placeholder.
    const std::string jsPlaceholder = "JS_PLACEHOLDER";
    size_t jsPos                    = rawHtml.find(jsPlaceholder);
    if (jsPos != std::string::npos)
    {
        rawHtml.replace(jsPos, jsPlaceholder.length(), rawJs);
    }
    else
    {
        LOG_ERROR(logging::g_qSharedLogger, "VisualizationHandler: JS_PLACEHOLDER not found in WebVisualizer.html.");
        return "";
    }
