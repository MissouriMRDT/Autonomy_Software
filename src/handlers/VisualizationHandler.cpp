...
std::vector<char> VisualizationHandler::OnRequestPointCloud(const std::string& szQuery)
{
    ...
    pubsub::SharedSnapshot<cv::Mat> pFrontCloudSnapshot = m_rdFrontPointCloud.Get();
    pubsub::SharedSnapshot<cv::Mat> pRearCloudSnapshot  = m_rdRearPointCloud.Get();
    ...
    fnAppendPointCloud(pFrontCloudSnapshot->tData, vBuffer, nPointCloudDensity);
    fnAppendPointCloud(pRearCloudSnapshot->tData, vBuffer, nPointCloudDensity);
    return vBuffer;
}
