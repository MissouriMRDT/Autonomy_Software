...
namespace constants
{
    ...
    // Control constants.
    const double DRIVE_POWER_PID_PROPORTIONAL = 0.01;      // The proportional gain for the controller used to point the rover at a goal speed during navigation.
    const double DRIVE_POWER_PID_INTEGRAL     = 0.0003;    // The integral gain for the controller used to point the rover at a goal speed during navigation.
    const double DRIVE_POWER_PID_DERIVATIVE   = 0.008;     // The derivative gain for the controller used to point the rover at a goal speed during navigation.
    const double DRIVE_POWER_PID_FEEDFORWARD  = 0.0;       // The feedforward for the controller used to predict control output.
    const double DRIVE_POWER_PID_MAX_ERROR    = 0.0;    // The max allowable error the controller will see per iteration. This is on degrees from setpoint. 0 = Disable.
    const double DRIVE_POWER_PID_MAX_INTEGRAL_TERM = 0.0;    // The max effort the I term is allowed to contribute. 0 = Disable.
    const double DRIVE_POWER_PID_MAX_RAMP_RATE     = 0.0;    // The max ramp rate of the output of the PID controller. 0 = Disable.
    const double DRIVE_POWER_PID_OUTPUT_FILTER     = 0.0;    // Larger values will filter out large spikes or oscillations. 0.1 is a good starting point. 0 = Disable.
    const double DRIVE_POWER_PID_TOLERANCE       = 0.0;    // The max allowable error from the setpoint for the controller to be considered at the setpoint. 0 = Disable.
    const bool DRIVE_POWER_PID_OUTPUT_REVERSED   = false;     // Negates the output of the PID controller.
    const double DRIVE_STEERING_PID_PROPORTIONAL = 0.01;      // The proportional gain for the controller used to point the rover at a goal heading during navigation.
    const double DRIVE_STEERING_PID_INTEGRAL     = 0.0003;    // The integral gain for the controller used to point the rover at a goal heading during navigation.
    const double DRIVE_STEERING_PID_DERIVATIVE   = 0.008;     // The derivative gain for the controller used to point the rover at a goal heading during navigation.
    const double DRIVE_STEERING_PID_FEEDFORWARD  = 0.0;       // The feedforward for the controller used to predict control output.
    const double DRIVE_STEERING_PID_MAX_ERROR = 180.0;    // The max allowable error the controller will see per iteration. This is on degrees from setpoint. 0 = Disable.
    const double DRIVE_STEERING_PID_MAX_INTEGRAL_TERM = 0.1;     // The max effort the I term is allowed to contribute. 0 = Disable.
    const double DRIVE_STEERING_PID_MAX_RAMP_RATE     = 0.08;    // The max ramp rate of the output of the PID controller. 0 = Disable.
    const double DRIVE_STEERING_PID_OUTPUT_FILTER     = 0.0;     // Larger values will filter out large spikes or oscillations. 0.1 is a good starting point. 0 = Disable.
    const double DRIVE_STEERING_PID_TOLERANCE     = 0.0;    // The max allowable error from the setpoint for the controller to be considered at the setpoint. 0 = Disable.
    const bool DRIVE_STEERING_PID_OUTPUT_REVERSED = false;    // Negates the output of the PID controller.
    ...
    const std::string OBJECTDETECT_MAINCAM_TORCH_MODEL =
        "../data/models/yolo_models/bmp_v4/v8n_x640_200epochs/best.torchscript";    // The model path to use for detection.
